# import alto # Disabled - Local implementation used
# from alto import parse_file # Disabled - Local implementation used
import os
...
class NewAltoParser:
    ...
    def extract_text_blocks(self):
        ...
        for text_block in root.findall(".//TextBlock"):
            ...
            data.update(
                {key.lower(): value for key, value in text_block.attrib.items()}
            )
            ...
            for text_line in text_block.findall(".//TextLine"):
                line_data = {
                    key.lower(): value for key, value in text_line.attrib.items()
                }
...
class NewAltoReader:
        ...
        if self.page_image is None:
            ...
            for possible_format in possible_formats:
                full_path = os.path.join(
                    os.path.dirname(os.path.dirname(self.page_xml_path)),
                    f"MASTER/{possible_format}.jp2",
                )
                if os.path.exists(full_path):
                    break
            else:
                raise Exception(
                    f"Page file naming format unknown, tried: {possible_formats}. See if another format is in MASTER/xxx.jp2"
                )
    ...
    def get_image_for_block(self, block, padding=0):
        ...
        return image.crop(
            [
                crop_coordinates[0] - padding,
                crop_coordinates[1] - padding,
                crop_coordinates[2] + padding,
                crop_coordinates[3] + padding,
            ]
        )
    ...
    def extract_block_data(self, block):
        return {
            ...
            "position": (
                float(block.hpos),
                float(block.vpos),
                float(block.hpos) + float(block.width),
                float(block.vpos) + float(block.height),
            ),
            "block_id": block.id,
            "tagrefs": block.tagrefs,
            "type": self.tag_types.get(block.tagrefs, "Unknown"),
    ...
    def get_debug_image(self):
        ...
        for i, block in enumerate(blocks):
            draw.text(
                (
                    int(block["position"][0] / res_downsample),
                    int(block["position"][1] / res_downsample) - 40,
                ),
                f"{i} {block['group']} {block['type']}",
                (255, 100, 100),
                font=DEFAULT_FONT,
            )
            draw.rectangle(
                [int(c / res_downsample) for c in block["position"]],
                outline=(255, 0, 0),
                width=3,
            )
        return image
                ...
                if div.get("TYPE") == "CONTENT":
                    for d in div:
                        ...
                        if d_body_content is not None and len(d_body_content) > 0:
                            ...
                            print(
                                f"Article {article_id} does not have content and was skipped."
                            )
    return mets_data
